...
exactly as v0.14.x did. A subscription that asks for no tag receives
byte-identical messages on every channel.

Everything below is opt-in work.
...
2. Update the dependency to `~> 0.15.0` and run `mix deps.get`.
